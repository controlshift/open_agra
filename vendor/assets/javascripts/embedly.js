 ...
 * resize_content:   null              // content css on which resize event is to be triggered
 *
...
(function ($) {
  ...
  $.extend({
    embedly: function (urls, options, callback) {
      var elems = [];
      var settings;
      options = options ? options : {url: "http://api.embed.ly/"};
      settings = $.extend({}, $.embedly.defaults, options);
      var path = settings.url;
      if (!settings.urlRe) {
        settings.urlRe = window.embedlyURLre;
      }
      ...
      if (!settings.success) {
        settings.success = function (oembed, dict) {
          ...
          else if (_a === 'replaceParent') {
            elem.parent().replaceWith(oembed.code);
          }
          if (settings.resize_content) {
            $(settings.resize_content).resize();
          }
        };
      }
      if (!settings.error) {
      ...
      var getParams = function (urls) {
        ...
        if (settings.maxWidth) {
          _p += '&maxwidth=' + settings.maxWidth;
        }
        else if (typeof dimensions !== "undefined") {
          _p += '&maxwidth=' + dimensions.width;
        }
        if (settings.maxHeight) {
          _p += '&maxheight=' + settings.maxHeight;
        }
        if (settings.chars) {
          _p += '&chars=' + settings.chars;
        }
        if (settings.words) {
          _p += '&words=' + settings.words;
        }
        if (settings.secure) {
          _p += '&secure=true';
        }
        ...
        if (typeof settings.autoplay === "string" || typeof settings.autoplay === "boolean") {
          _p += "&autoplay=" + settings.autoplay;
        }
        if (settings.width) {
          _p += "&width=" + settings.width;
        }
        return _p;
      };
      var getUrl = function () {
      ...
      var processEmbed = function (oembed, dict) {
        ...
        if ((_a = oembed.type) === 'photo') {
          ...
          return settings.success(null, dict);
...
//               code += description;
        ...
        if (typeof dict.node !== "undefined") {
          $(dict.node).data('oembed', oembed).trigger('embedly-oembed', [oembed]);
        }
        return settings.success(oembed, dict);
      };
      ...
      var processBatch = function (batch) {
        ...
        $.ajax({
          ...
          cache: true,
          dataType: 'jsonp',
          jsonpCallback: 'embedlyJsonpSuccess',
          data: getParams(urls),
