module Moonshine
  module Redis
    ...
    def redis_supports_infinite_maxclients?
      major, minor, patch = *(configuration[:redis][:version] || '2.4.17').split('.')
      if (major.to_i <= 2 && minor.to_i <= 4)
        true
      else
    ...
    def redis(options={})
      ...
      grep_redis_version = if (major.to_i <= 2 && minor.to_i <= 4)
                             "grep 'Redis server version #{version}' "
                           else
                             "grep 'Redis server v=#{version} '"
                           end
      exec 'install redis',
        ...
        :unless => "test -f /usr/local/bin/redis-server && /usr/local/bin/redis-server --version | #{grep_redis_version}"
