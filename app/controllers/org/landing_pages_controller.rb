class Org::LandingPagesController < Org::OrgController
  before_filter :load_and_authorize_effort, only: [:edit, :update, :show]

  def index
    @efforts = Effort.paginate(page: params[:page]).order('created_at DESC').where(organisation_id: current_organisation.id, effort_type: 'open_ended')
  end
  ...
  def create
    ...
    if @effort.save
      redirect_to org_landing_page_path(@effort)
    else
      render :new
    end
  end
